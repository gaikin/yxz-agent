...
- 本次迭代采用“确认时快照”策略：点击 `全部执行` 时，仅放行当前弹窗里已展示的待执行项
- 执行过程中新增到点任务不插入当前执行队列，继续保留为 `pending`
- 当前队列执行完后，如仍有 `pending` 项，再重新更新右下角弹窗概览
- 弹窗进入“执行中”状态后，不允许再点击 `忽略`，也不允许手动关闭
- 当前确认队列执行完后，弹窗自动关闭
...
Note over DCF,POP: 点击“全部执行”只放行当前弹窗快照里的待执行项；执行中新增项留待下一轮确认

### 9.7 右下角弹窗规则汇总

规则汇总如下：

- 用户首次打开子窗体时完成一次统一自动执行授权
- 定时任务到点后由独立右下角弹窗展示待执行概览，不依赖子窗体是否打开
- DCF 内部维护 `pending execution item` 列表，不引入独立 `batch` 领域对象
- 同一 `scheduleId` 再次到点时新增新的 `execution item`
- 弹窗概览不做聚合，逐条展示 `任务名 + 触发时间`
- 用户点击 `全部执行` 时，只放行点击瞬间弹窗里展示的待执行项
- 被确认的待执行项默认按 `requestedAt` 升序串行执行
- 单条失败不阻断后续待执行项
- 执行过程中新增到点任务不插入当前执行队列，继续保留为 `pending`
- 当前确认队列执行完后，如仍有 `pending` 项，右下角弹窗立即再次展示最新概览
- 用户点击 `忽略` 或关闭弹窗时，当前展示的待执行项记为 `skipped`
- `skipped` 写入执行记录，并写入对应 `scheduleId` 的 `lastStatus`
- 当前版本对 `skipped` 只记录，不做额外提醒、告警或展示
- 点击 `全部执行` 后，弹窗进入“执行中”状态
- 执行中不允许再点击 `忽略`，也不允许手动关闭弹窗
- 当前确认队列执行完后，弹窗自动关闭
- 当前版本暂不实现自动过期机制；“是否引入过期时间”列为待确认事项
